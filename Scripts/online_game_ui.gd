...
@export var current_minutes: int:
	set(minutes_in):
		if minutes_in >= 0:
			current_minutes = minutes_in
		else: 
			current_minutes = 0
@export var current_seconds: int:
	set(seconds_in):
		if !OnlineGameManager.is_overtime:
			if seconds_in >= 0:
				current_seconds = seconds_in
				if current_minutes == 0 and current_seconds == 0:
					if multiplayer.is_server():
						OnlineGameManager.end_regulation.rpc()
			else:
				current_minutes -= 1
				current_seconds = 59
		elif OnlineGameManager.is_overtime:
			if seconds_in <= 59:
				current_seconds = seconds_in
			else:
				current_minutes += 1
...
func _ready() -> void:
	start_minutes = OnlineGameManager.game_length
	current_minutes = start_minutes
	current_seconds = start_seconds
	blue_score_label.text = "Blue: " + str(OnlineGameManager.team_left_score)
	red_score_label.text = "Red: " + str(OnlineGameManager.team_right_score)
	game_timer.start(1)
